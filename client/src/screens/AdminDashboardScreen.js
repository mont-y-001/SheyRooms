...
const ROOM_TYPES = ['Standard', 'Deluxe', 'Suite', 'Villa', 'Apartment', 'Cottage'];
...
export default function AdminDashboardScreen() {
  ...
  const fetchData = async () => {
    try {
      ...
      const [roomsRes, bookingsRes, requestsRes] = await Promise.all([
      ...
      const bookingsWithRooms = (bookingsRes.data || []).map(b => ({
        ...b,
          ...
          <div className="modal-dialog modal-lg modal-dialog-centered">
            <div className="modal-content" style={{ borderRadius: 20 }}>
              <div className="modal-header border-0">
                ...
                <button type="button" className="close" onClick={() => setShowRoomModal(false)}>
                  <span>&times;</span>
                </button>
              </div>
              <div className="modal-body">
                <div className="row">
                  <div className="col-md-6 mb-3">
                    ...
                    <input className="form-control" value={roomForm.name} onChange={e => setRoomForm(f => ({ ...f, name: e.target.value }))} />
                  </div>
                  <div className="col-md-6 mb-3">
                    <label className="form-label fw-semibold">Type</label>
                    <select className="form-control" value={roomForm.type} onChange={e => setRoomForm(f => ({ ...f, type: e.target.value }))}>
                      {ROOM_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
                    </select>
                  </div>
                  <div className="col-md-4 mb-3">
                    <label className="form-label fw-semibold">Max Guests</label>
                    <input type="number" className="form-control" value={roomForm.maxcount} onChange={e => setRoomForm(f => ({ ...f, maxcount: e.target.value }))} />
                  </div>
                  <div className="col-md-4 mb-3">
                    <label className="form-label fw-semibold">Rent/Day (₹)</label>
                    <input type="number" className="form-control" value={roomForm.rentperday} onChange={e => setRoomForm(f => ({ ...f, rentperday: e.target.value }))} />
                  </div>
                  <div className="col-md-4 mb-3">
                    <label className="form-label fw-semibold">Phone</label>
                    <input className="form-control" value={roomForm.phonenumber} onChange={e => setRoomForm(f => ({ ...f, phonenumber: e.target.value }))} />
                  </div>
                  <div className="col-12 mb-3">
                    <label className="form-label fw-semibold">Description</label>
                    <textarea className="form-control" rows={3} value={roomForm.description} onChange={e => setRoomForm(f => ({ ...f, description: e.target.value }))} />
                  </div>
                  <div className="col-12 mb-3">
                    <label className="form-label fw-semibold">Image URLs (one per line)</label>
                    <textarea className="form-control" rows={3} value={roomForm.imageurls} onChange={e => setRoomForm(f => ({ ...f, imageurls: e.target.value }))} placeholder="https://example.com/photo1.jpg" />
                  </div>
                </div>
              </div>
              <div className="modal-footer border-0">
                <button className="btn btn-outline-secondary" onClick={() => setShowRoomModal(false)}>Cancel</button>
                <button className="btn btn-dark" onClick={saveRoom} disabled={saving}>
